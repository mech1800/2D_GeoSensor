...
# 強フィルター用＋透過表示あり→stressも変更されるので注意
def mk_image(pre_geometry_dataset,geometry_dataset,contact_dataset,stress_dataset,force_dataset,label,max,number):

    # データ数を4の倍数にするために最後に0行列を複数個加える
    zeros = np.zeros_like((pre_geometry_dataset[0]))
    for i in range(4-len(pre_geometry_dataset)%4):
        pre_geometry_dataset = np.vstack((pre_geometry_dataset, [zeros]))
        geometry_dataset = np.vstack((geometry_dataset, [zeros]))
        contact_dataset = np.vstack((contact_dataset, [zeros]))
        stress_dataset = np.vstack((stress_dataset, [zeros]))
    ...
    # 横4×縦5(pre_geometry,geometry,contact,target contact,label)の画像を作成
    for i in range(len(pre_geometry_dataset)//4):
        ...
        ax1.imshow(pre_geometry_dataset[i*4],cmap=cm.jet)
        ax2.imshow(pre_geometry_dataset[i*4+1],cmap=cm.jet)
        ax3.imshow(pre_geometry_dataset[i*4+2],cmap=cm.jet)
        ax4.imshow(pre_geometry_dataset[i*4+3],cmap=cm.jet)
        ...
        ax1.set_title(str(i*4+1))
        ax2.set_title(str(i*4+2))
        ax3.set_title(str(i*4+3))
        ax4.set_title(str(i*4+4))

        ax1.set_ylabel('pre_geometry', fontsize=8)

        # geometry_dataset用
        ax7 = fig.add_subplot(5,4,5)
        ax8 = fig.add_subplot(5,4,6)
        ax9 = fig.add_subplot(5,4,7)
        ax10 = fig.add_subplot(5,4,8)

        ax7.imshow(geometry_dataset[i*4],cmap=cm.jet)
        ax8.imshow(geometry_dataset[i*4+1],cmap=cm.jet)
        ax9.imshow(geometry_dataset[i*4+2],cmap=cm.jet)
        ax10.imshow(geometry_dataset[i*4+3],cmap=cm.jet)
        ...
        # contact_dataset用
        ax13 = fig.add_subplot(5,4,9)
        ax14 = fig.add_subplot(5,4,10)
        ax15 = fig.add_subplot(5,4,11)
        ax16 = fig.add_subplot(5,4,12)

        ax13.imshow(contact_dataset[i*4],cmap=cm.jet)
        ax14.imshow(contact_dataset[i*4+1],cmap=cm.jet)
        ax15.imshow(contact_dataset[i*4+2],cmap=cm.jet)
        ax16.imshow(contact_dataset[i*4+3],cmap=cm.jet)
        ...
        ax13.set_ylabel('target_contact', fontsize=8)
        ...
        if label == 'stress':
            ...
            ax19 = fig.add_subplot(5,4,17)
            ax20 = fig.add_subplot(5,4,18)
            ax21 = fig.add_subplot(5,4,19)
            ax22 = fig.add_subplot(5,4,20)

            im19 = ax19.imshow(stress_dataset[i*4],cmap=cm.jet)
            im20 = ax20.imshow(stress_dataset[i*4+1],cmap=cm.jet)
            im21 = ax21.imshow(stress_dataset[i*4+2],cmap=cm.jet)
            im22 = ax22.imshow(stress_dataset[i*4+3],cmap=cm.jet)
        ...
        if label == 'force':
            ...
            im19 = ax19.imshow(force_dataset[i*4],cmap=cm.jet,vmin=0,vmax=max)
            im20 = ax20.imshow(force_dataset[i*4+1],cmap=cm.jet,vmin=0,vmax=max)
            im21 = ax21.imshow(force_dataset[i*4+2],cmap=cm.jet,vmin=0,vmax=max)
            im22 = ax22.imshow(force_dataset[i*4+3],cmap=cm.jet,vmin=0,vmax=max)
